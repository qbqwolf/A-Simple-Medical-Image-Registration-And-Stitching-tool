...
class Stitcher(Utility.Method):
    ...
    def flowStitch(self, fileList, caculateOffsetMethod):
        ...
        for fileIndex in range(0, fileNum - 1):
            ...
            if status == False and self.ishandle == False:
                ...
                while True:
                    ...
                    if match:
                        ...
                        print("Invalid input. Please enter offset as a list (e.g., [-974, -2]).")
            else:
                offsetList.append(offset)
                endfileIndex = fileIndex + 1
        endTime = time.time()
        ...
        stitchImage,roffset = self.getStitchByOffset(fileList, offsetList)
        endTime = time.time()
        self.printAndWrite("The time of fusing is " + str(endTime - startTime) + "s")

        if status == False:
            self.printAndWrite(describtion)
        return ((status, endfileIndex), roffset,inioffset,stitchImage)

    def flowStitchWithMutiple(self, fileList, caculateOffsetMethod,r,ch,outpath):
        """
        ...
        return result,roffset,inioffset
    ...
    def imageSetStitchWithMutiple(self, projectAddress, outputAddress, fileNum, caculateOffsetMethod, startNum = 1, fileExtension = "jpg",
                                  outputfileExtension = "jpg",r='R1',ch='ch1',outpath='./'):
        """
        功能：图像集多段拼接方法
        :param projectAddress: 项目地址
        :param outputAddress: 输出地址
        ...
        for i in range(startNum, fileNum+1):
            startTime = time.time()
            fileAddress = projectAddress + "\\" + str(i) + "\\"
            fileList = natsorted(glob.glob(fileAddress + "*." + fileExtension))
            if not os.path.exists(outputAddress):
                os.makedirs(outputAddress)
            Stitcher.outputAddress = outputAddress
            result,roffset,inioffset = self.flowStitchWithMutiple(fileList, caculateOffsetMethod,r,ch,outpath)
            self.tempImageFeature.isBreak = True
            if len(result) == 1:
    ...
    def getStitchByOffset(self, fileList, originOffsetList):
        ...
        return stitchResult.astype(np.uint8),offsetList
...
if __name__=="__main__":
    stitcher = Stitcher()
    imageA = cv2.imread(".\\demoimages\\dendriticCrystal\\1\\1-044.jpg", 0)
    imageB = cv2.imread(".\\demoimages\\dendriticCrystal\\1\\1-045.jpg", 0)
