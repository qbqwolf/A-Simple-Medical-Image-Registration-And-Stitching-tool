...
class Method():
    ...
    def detectAndDescribe(self, image, featureMethod):
        ...
        if self.isGPUAvailable == False: # CPU mode
            if featureMethod =="super":
                ...
                kps, features, heatmap = fe.run(img)
                kps=kps.T[:,:2]
                # features=features.T
    ...
    def matchDescriptors(self, featuresA, featuresB):
        ...
        if self.isGPUAvailable == False:        # CPU Mode
            ...
            elif self.featureMethod == "super":
                matches = nn_match_two_way(featuresA, featuresB, self.superthresh)
                matches = matches.T[:,:2].tolist()
                matches = [(int(y), int(x)) for x, y in matches]
